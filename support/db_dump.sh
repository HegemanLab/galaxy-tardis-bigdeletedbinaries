...
cd /export/backup/pg
if [ ! -d CVSROOT ]; then
  su -l -c "$CVS -d /export/backup/pg init"  postgres
fi
if [ ! -d dumpall ]; then
  su -l -c 'mkdir /export/backup/pg/dumpall' postgres
fi
...
if [ ! -d $PGDATA/CVS ]; then
  su -l -c "cd $PGDATA; $CVS -d /export/backup/pg co -d . dumpall" postgres
fi
